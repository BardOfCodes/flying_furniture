...
usage:
	blender blank.blend --background --python blender_render.py -- <save_file_loc> <base_name> <iteration> <obj_file>
inputs:
       <save_file_loc>: the folder where the rendered image is to be saved.
       <obj_file>: .obj file of the 3D shape model.
       <base_name>: output file for rendered images of this model.
       <iteration>: used to modify the base_name to get final filename.

changed by: bardofcodes
 
original authors: hao su, charles r. qi, yangyan li
'''
...
# number of renders:
no_renders = np.random.choice(np.arange(2,6))
view_params = []
# Not too much, not too little
# 95 % of the images in (-2*sigma,2*sigma)
azi = np.clip(np.random.normal(0,45,size=(no_renders,)),-360,360)
ele = np.clip(np.random.normal(0,18,size=(no_renders,)),-45,60)
tilt = np.clip(np.random.normal(0,5,size=(no_renders,)),-45,45)
dist = np.clip(np.random.normal(4.5,1.25,size=(no_renders,)),2,7)
view_params = [[azi[i],ele[i],tilt[i],dist[i]] for i in range(len(azi))]

syn_images_folder = sys.argv[-4]
...
for param in view_params:
    ...
    syn_file_postfix = '_'+str(obj_count) + '.png'
    bpy.data.scenes['Scene'].render.filepath = os.path.join(syn_images_folder,syn_file_name+ syn_file_postfix)
